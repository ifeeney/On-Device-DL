...
class_labels = ["Doing other things","Drumming Fingers","No Gesture","Pulling Hand In","Pulling Two Fingers In","Pushing Hand Away","Pushing Two Fingers Away","Rolling Hand Backward","Rolling Hand Forward","Shaking Hand","Sliding Two Fingers Down","Sliding Two Fingers Left","Sliding Two Fingers Right","Sliding Two Fingers Up","Stop Sign","Swiping Down","Swiping Left","Swiping Right","Swiping Up","Thumb Down","Thumb Up","Turning Hand Clockwise","Turning Hand Counterclockwise","Zoomig In With Full Hand","Zooming In With Two Fingers","Zooming Out With Full Hand","Zooming Out With Two Fingers"]
...
def hand_detection_livestream(predict=False, port: int = 0):
    ...
    if(predict):
        ...
        sequence = []
        interpreter = tflite.Interpreter('Downloads/bellas_model3.tflite')
    ...
    while(True):
        ...
        if(predict and (time_elapsed > 1./frame_rate)):
            ...
            if len(sequence) == 13:
                ...
                reshaped = np.array([np.zeros_like(sequence[0])] * 12 + sequence + [np.zeros_like(sequence[0])] * 12)
                ...
                for index in top3_indices:
                    returned.append(f"{class_labels[index]}, Prob: {round(result[0][index])}")
                #returned = class_labels[np.argmax(result, axis=-1).item()]
   ...
   try:
      if detection_result.hand_landmarks == []:
         ...
         for idx in range(len(hand_landmarks_list)):
            ...
            cv2.putText(annotated_image, probs[2],(30, 30), cv2.FONT_HERSHEY_DUPLEX,0.8, HANDEDNESS_TEXT_COLOR, FONT_THICKNESS, cv2.LINE_AA)
            cv2.putText(annotated_image, probs[1],(30, 60), cv2.FONT_HERSHEY_DUPLEX,0.8, HANDEDNESS_TEXT_COLOR, FONT_THICKNESS, cv2.LINE_AA)
            cv2.putText(annotated_image, probs[0],(30, 90), cv2.FONT_HERSHEY_DUPLEX,0.8, HANDEDNESS_TEXT_COLOR, FONT_THICKNESS, cv2.LINE_AA)
